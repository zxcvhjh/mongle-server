...
```json
{
  ...
  "fileKeyList": ["post/image1.jpg", "post/image2.jpg"],
...
**설명**: 게시글 목록 조회 (커서 기반 페이징)
**인증**: 선택 (인증 시 좋아요 등 개인화 정보 포함)
**권한**: 없음
...
**설명**: 지도 영역 내 오브젝트 조회 (그레인, 정적/동적 클라우드)
**인증**: 선택 (인증 시 조회 여부 등 개인화 정보 포함)
**권한**: 없음
...
**설명**: 파일 업로드를 위한 Presigned URL 발급
**인증**: 선택
**권한**: 없음
...
**설명**: 파일 조회를 위한 Presigned URL 발급 (테스트용 API)
**인증**: 선택
**권한**: 없음
